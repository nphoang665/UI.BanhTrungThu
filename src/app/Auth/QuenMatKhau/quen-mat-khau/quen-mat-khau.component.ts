...
import { AuthService, RequestData } from '../../services/auth.service';
...
import { LoadingService } from '../../../ADMIN/services/Loadding/loading.service';
const icon_User = `
...
export class QuenMatKhauComponent implements OnInit {
    ...
    GuiEmail() {
        ...
        if (email) {
            let optionOtp = email.includes('@') ? 'guiEmail' : 'guiSdt';
            let requestData: RequestData = {
                optionOtp: optionOtp,
                email: optionOtp == 'guiEmail' ? this.form.get('email')?.value : '',
                matKhauMoi: '',
            };
    ...
    LayLaiMatKhau() {
        ...
        this.khachHangServices.getAllKhachHang().subscribe(resultKh => {
            let khachHangEmail = optionOtp == 'guiSdt'
                ? resultKh.find(kh => kh.soDienThoai == email)?.email
                : email;
